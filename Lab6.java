  ...
  private void percolateDown(int downThis) {
    ...
    while (downThis < heapSize) {
      ...
      if (childPos < heapSize) {
        if (
          (rightChildOf(downThis) < heapSize) &&
          (dataSaham.get(childPos + 1).harga < dataSaham.get(childPos).harga)
        ) {
          childPos++;
        }
...
class maxHeap {
  ...
  private void percolateUp(int upThis) {
    ...
    int value = valueSaham.harga;
    while (upThis > 0 && value >= dataSaham.get(parent).harga) {
      if (value == dataSaham.get(parent).harga) {
        if (valueSaham.id > dataSaham.get(parent).id) {
          dataSaham.set(upThis, dataSaham.get(parent));
          upThis = parent;
          parent = parentOf(upThis);
        }
...
public class Lab6 {
  ...
  public static int getIDMedian(ArrayList<Saham> arr, int N) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      kiri.insert(arr.get(i));
      kanan.insert(kiri.poll());
      if (kanan.size() > kiri.size()) {
        kiri.insert(kanan.poll());
      }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < Q; i++) {
      ...
      if (q.equals("TAMBAH")) {
        ...
        data.add(newSaham);
        int med = getIDMedian(data, data.size());
